...
private:
    ...
    // Reusable scratch PlayerCheckpoint used to copy real player state into
    // sim player state with bit-exact fidelity. PlayerObject's own
    // saveToCheckpoint/loadFromCheckpoint methods capture ~80 physics
    // fields the engine treats as authoritative for respawn — including
    // m_yVelocityUnrounded, m_jumpBuffered, mode flags, gravity state,
    // dash state, slope state, collision state — many of which copyAttributes
    // doesn't reach. This pair is what closes the button-hold-duration
    // sync gap that produced Death 1 (sim yVel=0.1, real yVel=4.86 because
    // real had been holding several ticks already and copyAttributes wasn't
    // forwarding that physics state). Allocated lazily in runPlan, retained,
    // reused across all candidates this level. Released in onPlayLayerQuit.
    PlayerCheckpoint*    m_syncCheckpoint{nullptr};
    PlayerCheckpoint*    m_syncCheckpoint2{nullptr};  // for P2 in dual mode
