        ...
        if (idx >= 0 && predIdx < m_best.samples.size()) {
            ...
            if (distSq > kDivergenceLogDistSq) {
                ...
                if ((m_frame - m_lastDivergenceLogFrame) >= 60) {
                    ...
                    // Also stamp with [TEL] so the harness picks it up as
                    // a structured event alongside positions/triggers/deaths.
                    geode::log::warn(
                        "[divergence] [TEL] divergence tick={} dist={:.3f} (full state in divergence.log)",
