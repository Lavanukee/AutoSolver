...
struct LayerStateSnapshot {
    ...
    // GJGameState's layer-level "current time-mod" cache. Bindings at
    // GeometryDash.bro:8978-8979, neighbours of m_levelFlipping/m_isDualMode/
    // m_lastActivatedPortal1/2 — i.e. the same family of fields the engine
    // mutates as portals are crossed and reads back per tick when computing
    // current effective speed. Suspected leak path for the speed-mod portal:
    // sim's traversal mutates these, our snapshot didn't restore them, the
    // engine's next-frame recompute (GJBaseGameLayer::update +0xe8) reads
    // the mutated value and writes it to the real players. See
    // docs/issue-speed-portal-leak.md.
    float            timeModRelated;
    bool             timeModRelated2;
