...
void TrajectorySimulator::onPlayLayerQuit() {
    ...
    if (m_simP1OwnRings) { m_simP1OwnRings->release(); m_simP1OwnRings = nullptr; }
    if (m_simP2OwnRings) { m_simP2OwnRings->release(); m_simP2OwnRings = nullptr; }
    // Release scratch checkpoints — re-created lazily on next level's
    // first runPlan/runBranch call. Holding across level changes is
    // wrong because a PlayerCheckpoint may reference level-specific
    // GameObjects (m_objectSnappedTo, m_lastGroundObject, etc.) that
    // get freed when the level unloads.
    if (m_syncCheckpoint)  { m_syncCheckpoint->release();  m_syncCheckpoint  = nullptr; }
    if (m_syncCheckpoint2) { m_syncCheckpoint2->release(); m_syncCheckpoint2 = nullptr; }
    m_simP1 = nullptr;
    m_simP2 = nullptr;
    m_pl = nullptr;
    m_simulating = false;
...
void TrajectorySimulator::runBranch(PlayerObject* sim, PlayerObject* base,
    ...
    // PlayerCheckpoint-based bit-exact state sync. The engine's own
    // save/load checkpoint pair captures ~80 physics fields that respawn
    // logic relies on — many that the explicit field-by-field block below
    // doesn't reach (m_yVelocityUnrounded, m_groundYVelocity,
    // m_yVelocityBeforeSlope, m_lastFlipTime, m_objectSnappedTo,
    // m_lastCollisionBottom/Top/Left/Right, etc.). Without this, the sim
    // player's physics state diverges from real on edge cases — exactly
    // the Death 1 signature observed (real had been holding for several
    // ticks at max ship yVel; sim started fresh at yVel near zero). The
    // checkpoint pair is allocated once per simulator lifetime, reused
    // for every sim run.
    if (!m_syncCheckpoint) {
        m_syncCheckpoint = PlayerCheckpoint::create();
        if (m_syncCheckpoint) m_syncCheckpoint->retain();
    }
    if (m_syncCheckpoint) {
        base->saveToCheckpoint(m_syncCheckpoint);
        sim->loadFromCheckpoint(m_syncCheckpoint);
    }
    ...
    for (int i = 0; i < m_iterations; ++i) {
        ...
        if (m_triggers && m_pl->m_effectManager) {
            ...
            fx->prepareMoveActions(m_frameDt, false);
            fx->processMoveCalculations();
            fx->postMoveActions();
        }
...
PlanResult TrajectorySimulator::runPlan(PlayerObject* base1, PlayerObject* base2,
    ...
    auto initSim = [this](PlayerObject* sim, PlayerObject* base, bool isP2) {
        sim->setVisible(false);
        sim->copyAttributes(base);
        // PlayerCheckpoint-based bit-exact state sync — see runBranch above
        // for the rationale. We use two checkpoint scratch instances (one
        // per sim player slot) so a dual-mode runPlan can sync both halves
        // independently. Allocated once per simulator lifetime, retained,
        // reused across all runPlan candidates.
        PlayerCheckpoint*& cp = isP2 ? m_syncCheckpoint2 : m_syncCheckpoint;
        if (!cp) {
            cp = PlayerCheckpoint::create();
            if (cp) cp->retain();
        }
        if (cp) {
            base->saveToCheckpoint(cp);
            sim->loadFromCheckpoint(cp);
        }
        sim->m_gravityMod = base->m_gravityMod;
        sim->m_isOnGround = base->m_isOnGround;
    ...
    initSim(simA, base1, /*isP2=*/false);
    if (simB) initSim(simB, base2, /*isP2=*/true);
    clearActivated();
    clearSimDestroyed();
